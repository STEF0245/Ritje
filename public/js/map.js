...
const schoolConfig = window.__RITJE_SCHOOL_MARKER_DATA__ || null
...
class AppMap {
	...
	getSchoolMarkerData() {
		const data = {
			...
			mapsUrl:
				schoolConfig?.mapsUrl ||
				this.generateGoogleMapsLink(
					schoolConfig?.coords?.lat,
					schoolConfig?.coords?.lon
				),
			icon: 'fa-school'
		}
